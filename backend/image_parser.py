...
- sales_text: 이미지에서 구매 수량을 나타내는 숫자를 "N만명+", "N천명+", "N명+" 형식으로 추출 (예: "4만명+", "2천명+", "100명+"). 쿠팡의 "N명이 구매했어요", "N만명 구매" 등 다양한 표현 포함. 확인 불가하면 null
...
def parse_sales_estimate(sales_text: Optional[str]) -> Optional[int]:
    ...
    [수정] 정규식으로 숫자+단위만 추출 → 부가 문자열이 붙어도 파싱 성공
    이전 방식: "4만명+ 구매".replace("만","") → "4 구매" → float() 실패
    새 방식: re.search로 "4만" 만 잡아냄
    """
    if not sales_text:
        return None

    import re
    # "4만", "2.5천", "100" 등 숫자+단위 패턴 추출
