...
3. "구매 N위" 배지 (주황색/빨간색 배지에 순위 표기) — 없을 수도 있음
4. 브랜드명 + 별점/리뷰 수
5. 상품명 (전체 텍스트)
6. 상품명 바로 아래 "카테고리명 구매 N위 >" 형태의 순위 링크 — 배지 대신 이 형태로 표기되기도 함
7. 판매가격
8. 단위 단가 (예: "100ml당 1,200원", "1정당 100원")
...
추출 규칙:
- rank: 아래 두 가지 패턴 중 하나에서 숫자만 추출 (매우 중요)
  패턴1) 주황/빨간 배지에 "구매 N위" 표기 → N 추출 (예: "구매 1위" → 1)
  패턴2) 상품명 바로 아래 줄에 "카테고리명 구매 N위 >" 형태의 파란 링크 텍스트
         예시: "종합영양제 구매 10위 >" → 10, "세탁세제 구매 3위 >" → 3
         이 줄에서 숫자만 추출할 것
  두 패턴 모두 없으면 null
...
- sales_text: "한달간 N명 이상 구매했어요", "N만명+", "N천명+", "N명+" 등 구매 수량 관련 텍스트만 추출. 콤마 포함 숫자(예: "3,000명")도 그대로 추출. 확인 불가하면 null
  ⚠️ 주의: "N명 이상 만족했어요", "N명 만족" 등 만족도 관련 텍스트는 구매 수가 아니므로 반드시 null로 처리할 것
...
IMAGE_PARSE_MODEL = "claude-sonnet-4-6"
